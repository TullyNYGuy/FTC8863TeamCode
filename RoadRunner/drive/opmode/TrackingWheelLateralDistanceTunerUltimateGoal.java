...
@Config
@TeleOp(group = "drive")
public class TrackingWheelLateralDistanceTunerUltimateGoal extends LinearOpMode {
    public static int NUM_TURNS = 10;

    @Override
    public void runOpMode() throws InterruptedException {
        //MecanumDriveUlimateGoal drive = new MecanumDriveUlimateGoal(hardwareMap);
        ...
        if (!(drive.getLocalizer() instanceof TrackingWheelLocalizerUltimateGoal)) {
            RobotLog.setGlobalErrorMsg("StandardTrackingWheelLocalizer is not being set in the "
                    + "drive class. Ensure that \"setLocalizer(new StandardTrackingWheelLocalizer"
                    + "(hardwareMap));\" is called in MecanumDriveUlimateGoal.java");
        }
        ...
        telemetry.addLine("Effective LATERAL_DISTANCE: " +
                (headingAccumulator / (NUM_TURNS * Math.PI * 2)) * TrackingWheelLocalizerUltimateGoal.LATERAL_DISTANCE);
